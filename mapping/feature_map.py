...
class OneMap:
    ...
    def update(self,
        ...
        if len(values.shape) == 1 or (values.shape[-1] == 1 and values.shape[-2] == 1):
            ...
            # depth가 이미 CUDA 텐서인지 확인하여 불필요한 변환 방지
            if isinstance(depth, torch.Tensor) and depth.is_cuda:
                depth_tensor = depth
            else:
                depth_tensor = torch.from_numpy(depth).to("cuda") if isinstance(depth, np.ndarray) else depth.to("cuda")
            (confidences_mapped, values_mapped,
             obstacle_mapped, obstcl_confidence_mapped) = self.project_dense(values, depth_tensor,
